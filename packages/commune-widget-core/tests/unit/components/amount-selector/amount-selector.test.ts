...
describe('Amount selector component - commune-resource-selector', () => {
  afterEach(() => {
    fixtureCleanup();
  });

  it('is defined', () => {
    const el = document.createElement('commune-resource-selector');
    assert.instanceOf(el, AmountSelector);
  });

  it('displays account balance correctly', async () => {
    const el = await fixture<AmountSelector>(
      html` <commune-resource-selector></commune-resource-selector>`
    );
  ...
  it('resets input and acc balance on resource change', async () => {
    ...
    const el = await fixture<AmountSelector>(
      html`<commune-resource-selector
        .resources=${resources}
